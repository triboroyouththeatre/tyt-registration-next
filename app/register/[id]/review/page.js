...
function ReviewForm({ programId }) {
  ...
  // Cart state — other participants already added this session
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: p }, { data: prog }, { data: allParticipants }] = await Promise.all([
        supabase.from('participants').select('first_name, last_name, yog').eq('id', participantId).single(),
        supabase.from('programs').select(`
          label, fee, deposit_amount, balance_due_date,
          yog_min, yog_max, costume_fee, other_fee, other_fee_label,
      ...
      // Find eligible siblings (not current participant, within grade range)
  ...
  async function handleProceedToPayment() {
    ...
    if (!healthData) {
      setError('Health information is missing. Please go back and complete the health form.');
      setSubmitting(false);
      return;
    }
    if (!agreementData) {
      setError('Agreement signatures are missing. Please go back and sign the agreements.');
      setSubmitting(false);
      return;
    }
  ...
  function handleAddAnotherParticipant(siblingId) {
    // Save current participant to cart first
    addToCart();
  ...
  // All cart items including current one for display
  const allCartItems = [
  ...
  return (
    <div>
      ...
      <p style={{ fontFamily: 'var(--font-accent)', fontStyle: 'italic', color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
        Please review everything below before proceeding to payment.
      </p>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', marginBottom: '1rem' }}>
        ...
        <div style={{ background: 'var(--bg-hover)', borderBottom: '1px solid var(--border)', padding: '0.875rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-faint)', marginBottom: '0.15rem' }}>
              Registration
            </p>
        ...
        <div style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: 'var(--text-primary)' }}>{formatCurrency(program?.fee || 0)}</span>
            </div>
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: '0.5rem', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Total</span>
          ...
          <div style={{ background: 'var(--bg-hover)', borderRadius: 'var(--radius-sm)', padding: '0.875rem 1rem', display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
            <div>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-faint)', marginBottom: '0.2rem' }}>Due Today (Deposit)</p>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 800, color: 'var(--gold)' }}>{formatCurrency(deposit)}</p>
            </div>
            <div>
              ...
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatCurrency(balance)}</p>
            </div>
        ...
        {healthData && (
          <div style={{ borderTop: '1px solid var(--border)', padding: '1rem 1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
              {[
                { label: 'Academic', flag: healthData.academic_flag },
                { label: 'Behavioral', flag: healthData.behavioral_flag },
                { label: 'Allergies', flag: healthData.allergies_flag },
                { label: 'Asthma', flag: healthData.asthma },
                { label: 'Concussion', flag: healthData.concussion_flag },
              ].map(item => (
        ...
        {agreementData && (
          <div style={{ borderTop: '1px solid var(--border)', padding: '1rem 1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              <a href={`/register/${programId}/agreements?participant=${participantId}`} style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--gold)', textDecoration: 'none' }}>
                Edit
              </a>
            </div>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              Signed as <strong style={{ color: 'var(--text-primary)' }}>{agreementData[0]?.agreed_by}</strong> · {new Date(agreementData[0]?.agreed_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
            </p>
          </div>
        )}
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.875rem' }}>
          ...
          <label htmlFor="financial_aid" style={{ cursor: 'pointer' }}>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
              Check this box if you plan to submit a financial aid application. You will still need to pay the deposit today. TYT will adjust your balance upon review of your application.
            </p>
          </label>
        </div>
      </div>

      {/* Cart summary — if other participants already added */}
      ...
      {eligibleSiblings.length > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px dashed var(--border)', borderRadius: 'var(--radius-md)', padding: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {eligibleSiblings
              .filter(s => !cartItems.some(c => c.participantId === s.id))
              .map(s => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button
                    type="button"
                    onClick={() => handleAddAnotherParticipant(s.id)}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-dark)' }}>
      <nav style={{
        ...
        <a
          href={`/register/${programId}/agreements`}
          style={{
