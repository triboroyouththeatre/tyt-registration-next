    ...
    async function load() {
      try {
        ...
        const { data: prog } = await supabase
          .from('programs')
          .select('id, label, fee, deposit_amount, balance_due_date, session_id')
          .eq('id', programId)
          .single();
        ...
        if (prog?.session_id) {
          const { data: sess } = await supabase
            .from('sessions').select('season_id').eq('id', prog.session_id).single();
          if (sess?.season_id) {
            const { data: season } = await supabase
              .from('seasons').select('name, display_name').eq('id', sess.season_id).single();
            seasonStr = season?.display_name || season?.name || '';
          }
        }

        // Fetch most recent registrations for this family + program
        // by joining through cart_id — get all regs in the same cart as the participant's latest reg
        const { data: participantReg } = await supabase
          .from('registrations')
          .select('cart_id, registered_at')
        ...
        if (!participantReg) throw new Error('No registration found for participant');

        // Get all registrations in the same cart
        const { data: regs, error: regErr } = await supabase
          ...
          `)
          .eq('cart_id', participantReg.cart_id)
          .eq('family_id', profile.family_id);

        if (regErr || !regs?.length) throw new Error('Could not load registrations');
        ...
        {status === 'error' && (
          <div>
            <div className="tyt-error" style={{ marginBottom: '1rem' }}>
              Could not load your registration details. Your payment was processed — please contact us at admin@triboroyouththeatre.org if you need assistance.
            </div>
        ...
        {status === 'ready' && (
          <>
            ...
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: '#0a1a0a', border: '2px solid #22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem', fontSize: '1.5rem', color: '#22c55e' }}>
                ✓
              </div>
                ...
                <div key={reg.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', marginBottom: '1.25rem' }}>
                  ...
                  <div style={{ background: 'var(--bg-hover)', borderBottom: '1px solid var(--border)', padding: '0.875rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      {registrations.length > 1 && (
                        <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-faint)', marginBottom: '0.15rem' }}>
                          Registration {i + 1} of {registrations.length}
                        </p>
                      )}
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', fontWeight: 700, color: 'var(--gold)' }}>
                  ...
                  <div style={{ padding: '1.25rem' }}>
                    ...
                    <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
                      ...
                      {balance > 0 && <>
                        ...
                        {program?.balance_due_date && <Row label="Balance Due Date" value={fmtDate(program.balance_due_date)} />}
                      </>}
